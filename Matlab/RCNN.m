...
% Procesing Video Part show how this detection works on video
% Copyright 2017 Tony Huang
...
%% Processing Video
%The video is from https://www.youtube.com/watch?v=rHP-AJmX6HU
%It's clear that RCNN is not fast enough for realtime processing

doVideo = false;

stopSignVid = VideoReader('stop_sign_video.mp4');
nframes = stopSignVid.NumberofFrames;
I =  read(stopSignVid,1);
outputFrame = zeros([size(I,1) size(I,2) 3 nframes], class(I));

if doVideo
    

for k = 1:nframes
    singleFrame = read(stopSignVid,k);
    I = singleFrame;
    [bboxes, score] = detect(rcnn, I);
    % Display the detection results
    ...
    if ~isempty(bboxes) 
        ...
        outputFrame(:,:,:,k) = insertObjectAnnotation(I, 'rectangle', bbox, annotation);
    else
        outputFrame(:,:,:,k) = I;
    end
        
    disp(['Progress:', num2str(k*100/nframes),'%'])

end

v = VideoWriter('stop_sign_video_output.avi');
open(v)
...
close(v)
    ...
    % Load pre-trained network for the example.
    load('outputFrame.mat','outputFrame')       
end

frameRate = stopSignVid.FrameRate;
implay(outputFrame,frameRate);
